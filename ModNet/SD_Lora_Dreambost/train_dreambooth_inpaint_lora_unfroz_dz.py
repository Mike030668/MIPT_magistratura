...
class RectangleGenerator:
    ...
    def __call__(self, batch):
        #batch_size, num_channels, width, height = batch.size
        ...
        while not (self.min_rect_rel_square * sqr <=
                    (x2 - x1 + 1) * (y2 - y1 + 1) <=
                    self.max_rect_rel_square * sqr) and k<10000:
            x1, y1, x2, y2 = self.gen_coordinates(width, height)
            k+=1
...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        self.instance_prompt = pd.read_csv(instance_prompt)['caption'].values
    ...
    def __getitem__(self, index):
        example = {}
        idx_img = index % self.num_instance_images
        instance_image = Image.open(self.instance_images_path[idx_img])
        ...
        instance_prompt = self.instance_prompt[idx_img]
        ...
        if self.class_data_root:
            idx_img = index % self.num_class_images
            class_image = Image.open(self.class_images_path[idx_img])
            ...
            class_prompt = self.class_prompt[idx_img]
...
def main():
    ...
    if args.with_prior_preservation:
        ...
        if cur_class_images < args.num_class_images:
            ...
            for example in tqdm(
                ...
                fake_mask = RectangleGenerator(
                          min_rect_rel_square=0.1,
                          max_rect_rel_square=1
                          )(fake_pil_images)
    ...
    def collate_fn(examples):
        ...
        for example in examples:
            ...
            mask = RectangleGenerator(
                                      min_rect_rel_square=0.3,
                                      max_rect_rel_square=0.8
                                      )(pil_image)
    ...
    if accelerator.is_main_process:
        ...
        unet.save_pretrained(args.output_dir, from_pt=True) 
