...
1. Установи зависимости (см. верх блока в ноутбуке).
2. Подготовь влияющие ряды (папка/список кадров).
3. В ноутбуке укажи пути к ({\hat{x}_t}), выставь `SCENARIO`, `SEEDS`, `QTY_CADRS`.
4. Запусти `run_grid(experiment_matrix)` — логи и `results.csv` появятся в `results/...`.
...
* Статья (препринт/версия журнала): `[добавь ссылку]`
* Репозиторий с экспериментами: `[добавь ссылку]`
* Ноутбук экспериментов: `[Text2Video_project/additional_exp/Pipe_exp_K22_text_rote_dreif_latent_cropsquare_depth_.ipynb]`
